export function hasEnemyPiece(coords, myColor, board) {
  const enemyPieceColor = myColor === "b" ? "w" : "b";
  const [rowIdx, cIdx] = coords;
  if (board[rowIdx][cIdx] === null) return false;
  return board[rowIdx][cIdx][0] === enemyPieceColor;
}

export function hasOwnPiece(coords, myColor, board) {
  const [r, c] = coords;
  if (board[r][c] === null) return false;
  return board[r][c][0] === myColor;
}
